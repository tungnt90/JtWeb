	...
	$.fn.tabSwitch = function(command, Arguements, EndFunction) {
		...
		if(!$.isFunction(EndFunction)){
			...
			var Callback = function(){
				$('.slide-nav a.current').removeClass("current");
				$('.slide-nav a:eq(' + Args.index + ')').addClass("current");									
		...
		StoreToCache = function(){
			...
			$.data(DOMFirstObj, "id", tabSwitchIndex);
			...
			$.data(DOMFirstObj, "index", Args.index);
			$.data(DOMFirstObj, "speed", Args.speed);
		...
		backFromCache = function(){
			...
			var ViewPortObj = $.data(DOMFirstObj, "id");
		...
		var moveTo = function(){
			...
			if($.data(DOMFirstObj, "speed") >0) {
				Args.speed = $.data(DOMFirstObj, "speed"); 
			}
			var WraperSelector = "WideDiv" + $.data(DOMFirstObj, "id");				
			var WraperObj = $("#"+WraperSelector);
...
$(document).ready(function() {
	//$(".banner-slider").tabSwitch('create', {type: 'toggle', toggle:'fade', height: 375, width: 960});
	//$(".banner li").tabSwitch('create', {type: 'slide', height: 488, width: 907});
	//$(".banner li").tabSwitch("startAuto", {interval: 5000});
});
